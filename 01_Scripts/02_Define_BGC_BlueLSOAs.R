...
# Objectives
# Download Countries_December_2024_Boundaries_UK_BGC from the Open Geography Portal
# Dissolve countries
...
# (a) Read in the BGC countries spatial file
Coastline_BGC = st_read("../02_InputData/Countries_December_2024_Boundaries_UK_BGC_-9016689610564788328.gpkg")
Coastline_BGC
mapview(Coastline_BGC)

# (b) Read in 2021 BGC LSOA spatial file
LSOA21_BGC = st_read("../02_InputData/Lower_layer_Super_Output_Areas_December_2021_Boundaries_EW_BGC_V5_-6970154227154374572.gpkg")
LSOA21_BGC

# (c) Read in the Region BGC spatial file
Region_BGC = st_read("../02_InputData/Regions_December_2024_Boundaries_EN_BGC_-2638963433149857560.gpkg")
Region_BGC
...
buffer = st_read("../03_Buffers/BGC_Buffers/BGC_Buffer_5m.gpkg")
...
i=1
  ...
  LSOAIntersectsBGC.WithHoles = LSOAIntersectsBGC
...
# (8) Loop through remaining regions and rbind to LSOAIntersectsBGC
for (i in 2:9){
  ...
  if (nrow(LSOAIntersectsBGC) > 0) {
    ...
    LSOAIntersectsBGC.WithHoles = rbind(LSOAIntersectsBGC.WithHoles,LSOAIntersectsBGC)
...
#mapview(LSOAIntersectsBGC.WithHoles)
st_write(LSOAIntersectsBGC.WithHoles, "../10_LSOACoastalGeographies/BlueLSOAs_BGC_withHoles_withLondon.gpkg", append=FALSE)
LSOA21_BGC_BlueWithHoles_List = LSOAIntersectsBGC.WithHoles %>% st_drop_geometry() %>% select(LSOA21CD) %>% pull(.)
...
LSOA21_BGC_BlueNoHoles_List = FinalBGC %>% st_drop_geometry() %>% select(LSOA21CD) %>% pull(.)
...
LSOAIntersectsBGC.WithHoles.NoLondon = LSOAIntersectsBGC.WithHoles %>% dplyr::filter(!LSOA21CD %in% London.LSOA.List)
#mapview(LSOAIntersectsBGC.WithHoles.NoLondon)
...
LSOAIntersectsBGC.WithHoles.NoLondon_List = LSOAIntersectsBGC.WithHoles.NoLondon %>% st_drop_geometry() %>% select(LSOA21CD) %>% pull(.)
...
LSOA21_BGC_BlueNoHoles_NoLondon = FinalBGC %>% dplyr::filter(!LSOA21CD %in% London.LSOA.List)
#mapview(LSOA21_BGC_BlueNoHoles_NoLondon)
st_write(LSOA21_BGC_BlueNoHoles_NoLondon, "../10_LSOACoastalGeographies/BlueLSOAs_BGC_noHoles_noLondon.gpkg", append=FALSE)
LSOA21_BGC_BlueNoHoles_NoLondon_List = LSOA21_BGC_BlueNoHoles_NoLondon %>% st_drop_geometry() %>% select(LSOA21CD) %>% pull(.)
...
# Find BlueProximate LSOAs according to the BGC resolution
# This uses the FinalBGC Blue geography (i.e. no holes and with London)
Loops = length(Regions)

# Start with the first region (to set up output)
i = 1
Working.Blue.BGC.LSOAs = FinalBGC %>% dplyr::left_join(LSOA_Region, by = "LSOA21CD") %>% 
  dplyr::filter(RGN23CD == Regions[i])

RegionBox = st_bbox(Region_BGC[Region_BGC$RGN24CD == Regions[i],])
bbox_poly <- st_as_sfc(RegionBox)
RegionBox_extended <- st_buffer(bbox_poly, dist = 1000)
WorkingLSOAs = LSOA21_BGC[st_intersects(LSOA21_BGC, RegionBox_extended, sparse = FALSE), ]
#mapview(Region_BGC[Region_BGC$RGN24CD == Regions[i],]) + mapview(WorkingLSOAs)

# Extract LSOAs in the region (this will have holes)
Neighbours = WorkingLSOAs[Working.Blue.BGC.LSOAs, op = st_intersects] 
#mapview(Neighbours, col.regions = "blue") + mapview(Working.Blue.BGC.LSOAs, col.regions = "red")
...
#mapview(FinalBGC, col.regions = "lightblue", legend =FALSE)
...
st_write(FinalNeighbours,"../10_LSOACoastalGeographies/BlueProximateLSOAs_BGC_withHoles_withLondon.gpkg", append=FALSE)
FinalNeighbours_List = FinalNeighbours %>% st_drop_geometry() %>% select(LSOA21CD) %>% pull(.)
write.csv(FinalNeighbours_List,"../20_LSOACoastalLists/BlueProximateLSOAs_BGC_withHoles_withLondon.csv")


st_write(FinalBlueProximateLSOAs,"../10_LSOACoastalGeographies/BlueProximateLSOAs_BGC_noHoles_withLondon.gpkg", append=FALSE)
FinalBlueProximateLSOAs_List = FinalBlueProximateLSOAs %>% st_drop_geometry() %>% select(LSOA21CD) %>% pull(.)
write.csv(FinalBlueProximateLSOAs_List,"../20_LSOACoastalLists/BlueProximateLSOAs_BGC_NoHoles_withLondon_List.csv")
...
st_write(Neighbours.WithHoles.NoLondon, "../10_LSOACoastalGeographies/BlueProximateLSOAs_BGC_withHoles_noLondon.gpkg", append=FALSE)
Neighbours.WithHoles.NoLondon_List = Neighbours.WithHoles.NoLondon %>% st_drop_geometry() %>% select(LSOA21CD) %>% pull(.)
write.csv(Neighbours.WithHoles.NoLondon_List,"../20_LSOACoastalLists/BlueProximateLSOAs.withHoles.noLondon_List.csv", row.names = FALSE)


# Deal with the 'no holes' version
LSOA21_BGC_BlueProximateNoHoles_NoLondon = FinalBlueProximateLSOAs %>% dplyr::filter(!LSOA21CD %in% London.LSOA.List)
#mapview(LSOA21_BGC_BlueProximateNoHoles_NoLondon)
st_write(LSOA21_BGC_BlueProximateNoHoles_NoLondon, "../10_LSOACoastalGeographies/BlueProximateLSOAs_BGC_noHoles_noLondon.gpkg", append=FALSE)
LSOA21_BGC_BlueProximateNoHoles_NoLondon_List = LSOA21_BGC_BlueProximateNoHoles_NoLondon %>% st_drop_geometry() %>% select(LSOA21CD) %>% pull(.)
write.csv(LSOA21_BGC_BlueProximateNoHoles_NoLondon_List,"../20_LSOACoastalLists/BlueProximateLSOAs_BGC_noHoles_noLondon_List.csv", row.names = FALSE)


#################################################################################


LSOA21_BGC_BlueProximateNoHoles_NoLondon = st_read("../10_LSOACoastalGeographies/BlueProximateLSOAs_BGC_noHoles_noLondon.gpkg")
mapview(LSOA21_BGC_BlueProximateNoHoles_NoLondon)
