...
# 04a_LSOACentroidDistanceFrom_BGC_Coastline_1-20.R
...
# As set on lines 36-40, this script deals with buffers 1-20, which covers 40km to 30.5km

# Also see:
# 04b_LSOACentroidDistanceFrom_BGC_Coastline_21-40.R - covering 30km to 20.5km
# 04c_LSOACentroidDistanceFrom_BGC_Coastline_41-60.R - covering 20km to 10.5km
# 04d_LSOACentroidDistanceFrom_BGC_Coastline_61-80.R - covering 10Km to 500m
...
CoastalPoints = st_read("../04_CoastlinesAsPoints/BGC_Coastline_asPoints.gpkg")

# Get a list of all BGC buffer files and arrange in decsending size
# Note that the file.list also includes files for 300m, 200m and 100m but we won't process these
FileList = data.frame(Filename = list.files("../03_Buffers/BGC_Buffers/"))
FileList = FileList %>% mutate(Order = as.numeric(str_remove_all(Filename, "BGC_Buffer_|m.gpkg"))) %>% arrange(desc(Order))
#Loops = nrow(FileList)  
...
# Note that the Extended bbox (RegionBox_extended) is given CRS for National Grid (i.e. 27700)
 ...
 j = PreLoop
 buffer = st_read(paste0("../03_Buffers/BGC_Buffers/",FileList$Filename[j]))
...
#Extract = LSOA.df %>% select(1,2)
#Test = LSOA21_BGC %>% dplyr::left_join(Extract, by="LSOA21CD") %>% dplyr::filter(.[[10]] == 1)
...
#write.csv(LSOA.df,"../20_LSOACoastalLists/BGC_LSOA21_Distance_withLondon_withGaps_1-20.csv", row.names = FALSE)
write.csv(LSOA.df,paste0("../20_LSOACoastalLists/BGC_LSOA21_Distance_withLondon_withGaps_",FilenameSuffix,".csv"), row.names = FALSE)

#######################################################################################################
#######################################################################################################
#######################################################################################################

# Now we need to fill any holes at each level > to create BGC_LSOA21_Distance_withLondon_NoGaps.csv
...
LSOA21_BGC = LSOA21_BGC %>% dplyr::left_join(LSOA_Region, by = "LSOA21CD")
AllLSOAs = LSOA21_BGC %>% st_drop_geometry() %>% select(LSOA21CD)
...
for (i in 2:Loops){
  ...
  LSOASet = LSOA21_BGC %>% dplyr::left_join(Temp, by = "LSOA21CD") %>%
  ...
  if (nrow(WorkingLSOASet)>0){
    ...
    RegionLSOAs = LSOA21_BGC %>% dplyr::filter(RGN24CD == RegionList[j])
...
# Save to file
write.csv(AllLSOAs,paste0("../20_LSOACoastalLists/BGC_LSOA21_Distance_withLondon_NoGaps_",FilenameSuffix,".csv"), row.names = FALSE)

# Now we need to establish whether points are in London *or* can only be reached through the London region
# Need to create both BGC_LSOA21_Distance_NoLondon_withGaps.csv
#                 and BGC_LSOA21_Distance_NoLondon_NoGaps.csv
...
#Test = LSOA21_BGC %>% dplyr::left_join(TestDist, by = "LSOA21CD") %>% dplyr::filter(Buffer_20000m == 1)
#head(Test)
#mapview(Test) # i.e. all LSOAs in the 40000 buffer

# Extract polygon of London
Regions = st_read("../02_InputData/Regions_December_2024_Boundaries_EN_BGC_-2638963433149857560.gpkg")
...
for (k in 1:BufferLoops){
  ...
  WorkingBufferDist = as.numeric(str_remove_all(WorkingBufferName, "BGC_Buffer_|m"))
...
# Check
#TestDist = AllLSOAs %>% select(LSOA21CD,BGC_Buffer_40000m)
#TestMap = LSOA21_BGC %>% dplyr::left_join(TestDist, by = "LSOA21CD") %>% dplyr::filter(BGC_Buffer_40000m == 1)
...
# Check
#TestDist = NoLondonAllLSOAs %>% select(LSOA21CD,BGC_Buffer_40000m)
#TestMap = LSOA21_BGC %>% dplyr::left_join(TestDist, by = "LSOA21CD") %>% dplyr::filter(BGC_Buffer_40000m == 1)
#mapview(TestMap) # i.e. all LSOAs in the 40000 buffer

names(NoLondonAllLSOAs)
# And save files
#write.csv(NoLondonAllLSOAs,"../20_LSOACoastalLists/BGC_LSOA21_Distance_NoLondon_NoGaps_1-20.csv",row.names = FALSE)
write.csv(NoLondonAllLSOAs,paste0("../20_LSOACoastalLists/BGC_LSOA21_Distance_NoLondon_NoGaps_",FilenameSuffix,".csv"), row.names = FALSE)
