...
Region = st_read("Regions_December_2024_Boundaries_EN_BSC_-5107433749138478884.gpkg")
...
# Find points in the 40km Buffer - then loop down to 1km
# Look at points in each region in turn, and test against the buffer within a bbox + 40km in each direction

# Look at first region
FileList = data.frame(Filename = list.files("./Definitive/BSC_Buffers/"))
FileList = FileList %>% mutate(Order = as.numeric(str_remove_all(Filename, "Buffer_|Km.gpkg"))) %>% arrange(desc(Order))
Loops = nrow(FileList)  
...
i = 1
 ...
 # Find points with region bbox - not the extended version
 RegionPoints =  LSOA21_Centroids %>% dplyr::filter(RGN24CD == RegionList[i])
 #mapview(RegionPoints)

 # Find extended_buffer for region bbox
 j = 1
 buffer = st_read(paste0("./Definitive/BSC_Buffers/",FileList$Filename[j]))
 
 RegionBuffer = st_intersection(buffer,RegionBox_extended)
 #mapview(RegionBuffer)
 
 # Find points in region within buffer within region
 nrow(RegionPoints) # 4567
...
for (j in 2:Loops){
   print(paste0("Dealing with i = ",i," of 9 and j = ",j," of 40"))
...
# So LSOA.df has 0/1 for the first region
...
for (i in 2:41){
  Col=i
  print(sum(LSOA.df[,..Col]))
}
sum(LSOA.df[,2])  
    
# Check by plotting a few sets of points for the whole country
Extract = LSOA.df %>% select(LSOA21CD, Buffer_4Km)
Test = LSOA21_Centroids %>% dplyr::left_join(Extract, by="LSOA21CD") %>% dplyr::filter(Buffer_4Km == 1)
mapview(Test)
    
 
# Need to establish whether points in London *or* can only be reached through the London region



# - but we can't establish whether the 'route' to the
# sea goes through London - so we have to take a different approach
 ...
 for (i in 2:Loops) {
   ...
   print(paste0("Dealing with i = ",i," of ",Loops))
     ...
     if (dist > 500 & dist < 20000){
       ...
       #Addon = data.frame(X = coords$X[j], Y = coords$Y[j])
       #Addin = rbind(Addin,Addon)
 ...
 mapview(CoastalPoints) + mapview(Coastline_BSC)
 
 st_write(CoastalPoints,"./BSC_Coastline_asPoints.gpkg", append = FALSE)
 
# (2) Read in 2021 LSOA spatial centroids file 
LSOA21_Centroids = st_read("LLSOA_Dec_2021_PWC_for_England_and_Wales_2022_-7826745778656378855.gpkg")

# (3) Read in Region spatial file
...
# (5) Find LSOAs according to the BSC resolution ###############################
# 
