...
# 01_BlueLSOAs_BUA_and_PoI_and_Roads.R

# Establish whether Blue LSOAs, and their 50m, 100m, 200m, 300m, 400m & 500m coastal fringes,
# (a) lie within a BUA (and what category),
# (b) contain roads (and of what category), and
# (c) contain selected PoI
...
# (1) Read in the lists of Blue LSOAs###########################################
BlueLSOA_BFC_list = read.csv("./Definitive/LSOA21_BFC_Dist0_List.csv")
BlueLSOA_BGC_list = read.csv("./Definitive/LSOA21_BGC_Dist0_List.csv")
BlueLSOA_BSC_list = read.csv("./Definitive/LSOA21_BSC_Dist0_List.csv")


# (2) Read in the versions of the 2021 LSOA spatial file #######################
...
# (3) Read in dissolved coastlines (upon which to calculate buffers)
Dissolved_Coastline_BFC = st_read("./Definitive/Dissolved_Coastline_BFC.gpkg")
Dissolved_Coastline_BGC = st_read("./Definitive/Dissolved_Coastline_BGC.gpkg")
Dissolved_Coastline_BSC = st_read("./Definitive/Dissolved_Coastline_BSC.gpkg")
...
# Use Dissolved Coastlines to calculate 100m, 200m and 300m coastal buffers
# NB We already have calculated the 50m buffer

# BFC #######
buffer_Inner100 = st_buffer(Dissolved_Coastline_BFC, -100)
buffer_Outer100 = st_buffer(Dissolved_Coastline_BFC, 100)
buffer100 =  st_difference(buffer_Outer100,buffer_Inner100) 
st_write(buffer100, "./Definitive/BFC_CoastlineBuffer_100.gpkg", append=FALSE)

buffer_Inner200 = st_buffer(Dissolved_Coastline_BFC, -200)
buffer_Outer100 = st_buffer(Dissolved_Coastline_BFC, 100)
buffer200 =  st_difference(buffer_Outer100,buffer_Inner200) 
st_write(buffer200, "./Definitive/BFC_CoastlineBuffer_200.gpkg", append=FALSE)

buffer_Inner300 = st_buffer(Dissolved_Coastline_BFC, -300)
buffer_Outer100 = st_buffer(Dissolved_Coastline_BFC, 100)
buffer300 =  st_difference(buffer_Outer100,buffer_Inner300) 
st_write(buffer300, "./Definitive/BFC_CoastlineBuffer_300.gpkg", append=FALSE)

# and check
mapview(buffer300) + mapview(buffer200) + mapview(buffer100)


################################################################################

# Note that I am locating points in LSOAs using the BFC resolution

# Find LSOAs in Region[1] and then define those parts of each LSOA  within
# 100m, 200m or 300m of the coastline
LSOA21_BFC_Dist0 = st_read("./Definitive/LSOA21_BFC_Dist0.gpkg")
Buffer100 = st_read("./Definitive/BFC_CoastlineBuffer_100.gpkg")
Buffer200 = st_read("./Definitive/BFC_CoastlineBuffer_200.gpkg")
Buffer300 = st_read("./Definitive/BFC_CoastlineBuffer_300.gpkg")

# Deal with 1st region
LSOAList = LSOA_Region %>% filter(RGN23CD == Regions[1]) %>% select(LSOA21CD) %>% pull(.)
LSOAExtract = LSOA21_BFC_Dist0 %>% dplyr::filter(LSOA21CD %in% LSOAList)

WorkingRegion = Region_BFC %>% dplyr::filter(RGN24CD == Regions[1])
WorkingBuffer100 = st_intersection(WorkingRegion,Buffer100)
WorkingBuffer200 = st_intersection(WorkingRegion,Buffer200)
WorkingBuffer300 = st_intersection(WorkingRegion,Buffer300)

# Check
mapview(LSOAExtract, col.regions = "red") + mapview(WorkingBuffer100) + mapview(WorkingBuffer200) + mapview(WorkingBuffer300)

LSOA100 = st_intersection(LSOAExtract,WorkingBuffer100)
LSOA200 = st_intersection(LSOAExtract,WorkingBuffer200)
LSOA300 = st_intersection(LSOAExtract,WorkingBuffer300)

# Check
mapview(LSOA300) + mapview(LSOA200) + mapview(LSOA100)


# Look through remaining regions and rbind
for (i in 2:9) {
  print(paste0("Dealing with i = ",i," of 9"))
  ...
  TempLSOA100 = st_intersection(LSOAExtract,WorkingBuffer100)
  TempLSOA200 = st_intersection(LSOAExtract,WorkingBuffer200)
  TempLSOA300 = st_intersection(LSOAExtract,WorkingBuffer300)
  
  LSOA100 = rbind(LSOA100,TempLSOA100)
  LSOA200 = rbind(LSOA200,TempLSOA200)
  LSOA300 = rbind(LSOA300,TempLSOA300)
}

# Check
mapview(LSOA300) + mapview(LSOA200) + mapview(LSOA100)

# Save the coastline versions of the LSOAs
st_write(LSOA100, "./Definitive/BFC_LSOACoastLine100.gpkg", append=FALSE)
st_write(LSOA200, "./Definitive/BFC_LSOACoastLine200.gpkg", append=FALSE)
st_write(LSOA300, "./Definitive/BFC_LSOACoastLine300.gpkg", append=FALSE)



# Now we need to find PoI points in each Coastline LSOA ########################
LSOA100 = st_read("./Definitive/BFC_LSOACoastLine100.gpkg")
LSOA200 = st_read("./Definitive/BFC_LSOACoastLine200.gpkg")
LSOA300 = st_read("./Definitive/BFC_LSOACoastLine300.gpkg")


ONS_PoI = read_sf("./England_PoI.gpkg")
names(ONS_PoI)

ONS_PoI = ONS_PoI %>% dplyr::select(ref_no,name,pointx_class,groupname,categoryname,classname,
                             feature_easting,feature_northing)
names(ONS_PoI)
nrow(ONS_PoI)
head(ONS_PoI)

# Find which points intersect each LSOA coastline polygon - start with the 300m version
StartTime = Sys.time()
PoI_CoastLine300_LSOAs = st_intersection(ONS_PoI,LSOA300)
Sys.time() - StartTime  # 1.845248 mins
PoI_CoastLine300_LSOAs %>% st_drop_geometry() %>% select(LSOA21CD) %>% nrow(.) # 129743
st_write(PoI_CoastLine300_LSOAs, "./Definitive/PoI_CoastLine300_LSOAs.gpkg", append=FALSE)
PoI_CoastLine300_LSOA_List = PoI_CoastLine300_LSOAs %>% st_drop_geometry() %>% 
  select(LSOA21CD,pointx_class,groupname,categoryname,classname,feature_easting,feature_northing)
write.csv(PoI_CoastLine300_LSOA_List,"./Definitive/PoI_CoastLine300_LSOA_List.csv", row.names=FALSE)


# Now the 200 and 100 versions (which must be subsets of the 300)
StartTime = Sys.time()
PoI_CoastLine200_LSOAs = st_intersection(PoI_CoastLine300_LSOAs,LSOA200)
Sys.time() - StartTime  # 26.54064 secs
PoI_CoastLine200_LSOAs %>% st_drop_geometry() %>% select(LSOA21CD) %>% nrow(.) # 89673
st_write(PoI_CoastLine200_LSOAs, "./Definitive/PoI_CoastLine200_LSOAs.gpkg", append=FALSE)
PoI_CoastLine200_LSOA_List = PoI_CoastLine200_LSOAs %>% st_drop_geometry() %>% 
  select(LSOA21CD,pointx_class,groupname,categoryname,classname,feature_easting,feature_northing)
write.csv(PoI_CoastLine200_LSOA_List,"./Definitive/PoI_CoastLine200_LSOA_List.csv", row.names=FALSE)

# Now the 200 and 100 versions (which must be subsets of the 300)
StartTime = Sys.time()
PoI_CoastLine100_LSOAs = st_intersection(PoI_CoastLine300_LSOAs,LSOA100)
Sys.time() - StartTime  # 31.24589 secs
PoI_CoastLine100_LSOAs %>% st_drop_geometry() %>% select(LSOA21CD) %>% nrow(.) # 46021
st_write(PoI_CoastLine100_LSOAs, "./Definitive/PoI_CoastLine100_LSOAs.gpkg", append=FALSE)
PoI_CoastLine100_LSOA_List = PoI_CoastLine100_LSOAs %>% st_drop_geometry() %>% 
  select(LSOA21CD,pointx_class,groupname,categoryname,classname,feature_easting,feature_northing)
write.csv(PoI_CoastLine100_LSOA_List,"./Definitive/PoI_CoastLine100_LSOA_List.csv", row.names=FALSE)

# So we have the basis of Coastline LSOA descriptions using PoIs
# Come back to deal with these in due course

# Now the roads
st_layers("./oproad_gpkg_gb/Data/oproad_gb.gpkg")
MotorwayJunction = st_read("./oproad_gpkg_gb/Data/oproad_gb.gpkg", layer = "motorway_junction")
mapview(MotorwayJunction)

RoadLinks = st_read("./oproad_gpkg_gb/Data/oproad_gb.gpkg", layer = "road_link")
names(RoadLinks)
unique(RoadLinks$road_classification)
# "Unclassified"          "Unknown"               
# "B Road"                "A Road" 
# "Not Classified"        "Classified Unnumbered" 
# "Motorway"             

Motorway = RoadLinks %>% dplyr::filter(road_classification  == "MotorWay")
A_Road = RoadLinks %>% dplyr::filter(road_classification  == "A Road")
B_Road = RoadLinks %>% dplyr::filter(road_classification  == "B Road")
Classified_Road = RoadLinks %>% dplyr::filter(road_classification  == "Classified Unnumbered")
Unclassified_Road = RoadLinks %>% dplyr::filter(road_classification  == "Unclassified")
Not_Classified = RoadLinks %>% dplyr::filter(road_classification  == "Not Classified")

# This would probably run much faster if I split everything into regions - but I will run it overnight!

# Deal with LSOA300
LSOARoadLengths = data.frame(LSOA21CD = character(), Motorway = numeric(), A_Road = numeric(), B_Road = numeric(),
                      Classified_Road = numeric(), Unclassified_Road = numeric() , Not_Classified = numeric())
for (i in 1:nrow(LSOA300)){
  print(paste0("Dealing with i = ",i," of ",nrow(LSOA300)," at ",Sys.time()))
  LSOA = LSOA300$LSOA21CD[i]
  LSOA300_Motorway = sum(st_length(st_intersection(LSOA300[i,], Motorway)))
  LSOA300_A_Road = sum(st_length(st_intersection(LSOA300[i,], A_Road)))
  LSOA300_B_Road = sum(st_length(st_intersection(LSOA300[i,], B_Road)))
  LSOA300_Classified_Road = sum(st_length(st_intersection(LSOA300[i,], Classified_Road)))
  LSOA300_Unclassified_Road = sum(st_length(st_intersection(LSOA300[i,], Unclassified_Road)))
  LSOA300_Not_Classified = sum(st_length(st_intersection(LSOA300[i,], Not_Classified)))
  Addon = data.frame(LSOA21CD = LSOA, Motorway = as.numeric(LSOA300_Motorway), 
                     A_Road = as.numeric(LSOA300_A_Road), 
                     B_Road = as.numeric(LSOA300_B_Road), 
                     Classified_Road = as.numeric(LSOA300_Classified_Road),
                     Unclassified_Road = as.numeric(LSOA300_Unclassified_Road) , 
                     Not_Classified = as.numeric(LSOA300_Not_Classified))
  LSOARoadLengths = rbindlist(list(LSOARoadLengths,Addon))
}

# Find area covered
Hectares = data.frame(LSOA21CD = LSOA300$LSOA21CD, Hectares = as.numeric(st_area(LSOA300)/10000))
head(Hectares)
LSOARoadLengths = LSOARoadLengths %>% left_join(Hectares, by = "LSOA21CD")
head(LSOARoadLengths)
LSOARoadLengths = LSOARoadLengths %>% mutate(MotorwayPerHectare = Motorway/Hectares,
                                             A_RoadPerHectare = A_Road/Hectares,
                                             B_RoadPerHectare = B_Road/Hectares,
                                             Classified_RoadPerHectare = Classified_Road/Hectares,
                                             Unclassified_RoadPerHectare = Unclassified_Road/Hectares,
                                             Not_ClassifiedPerHectare = Not_Classified/Hectares,)
head(LSOARoadLengths)
LSOARoadLengths[1:5,c(1,8:14)]

write.csv(LSOARoadLengths, "./Definitive/LSOA300RoadLengths.csv", row.names = FALSE)


# Deal with LSOA200
LSOARoadLengths = data.frame(LSOA21CD = character(), Motorway = numeric(), A_Road = numeric(), B_Road = numeric(),
                             Classified_Road = numeric(), Unclassified_Road = numeric() , Not_Classified = numeric())
for (i in 1:nrow(LSOA200)){
  print(paste0("Dealing with i = ",i," of ",nrow(LSOA200)," at ",Sys.time()))
  LSOA = LSOA200$LSOA21CD[i]
  LSOA200_Motorway = sum(st_length(st_intersection(LSOA200[i,], Motorway)))
  LSOA200_A_Road = sum(st_length(st_intersection(LSOA200[i,], A_Road)))
  LSOA200_B_Road = sum(st_length(st_intersection(LSOA200[i,], B_Road)))
  LSOA200_Classified_Road = sum(st_length(st_intersection(LSOA200[i,], Classified_Road)))
  LSOA200_Unclassified_Road = sum(st_length(st_intersection(LSOA200[i,], Unclassified_Road)))
  LSOA200_Not_Classified = sum(st_length(st_intersection(LSOA200[i,], Not_Classified)))
  Addon = data.frame(LSOA21CD = LSOA, Motorway = as.numeric(LSOA200_Motorway), 
                     A_Road = as.numeric(LSOA200_A_Road), 
                     B_Road = as.numeric(LSOA200_B_Road), 
                     Classified_Road = as.numeric(LSOA200_Classified_Road),
                     Unclassified_Road = as.numeric(LSOA200_Unclassified_Road) , 
                     Not_Classified = as.numeric(LSOA200_Not_Classified))
  LSOARoadLengths = rbindlist(list(LSOARoadLengths,Addon))
}

Hectares = data.frame(LSOA21CD = LSOA200$LSOA21CD, Hectares = as.numeric(st_area(LSOA200)/10000))
head(Hectares)
LSOARoadLengths = LSOARoadLengths %>% left_join(Hectares, by = "LSOA21CD")
head(LSOARoadLengths)
LSOARoadLengths = LSOARoadLengths %>% mutate(MotorwayPerHectare = Motorway/Hectares,
...
write.csv(LSOARoadLengths, "./Definitive/LSOA200RoadLengths.csv", row.names = FALSE)
...
LSOARoadLengths = data.frame(LSOA21CD = character(), Motorway = numeric(), A_Road = numeric(), B_Road = numeric(),
                             Classified_Road = numeric(), Unclassified_Road = numeric() , Not_Classified = numeric())
for (i in 1:nrow(LSOA100)){
  print(paste0("Dealing with i = ",i," of ",nrow(LSOA100)," at ",Sys.time()))
  LSOA = LSOA100$LSOA21CD[i]
  LSOA100_Motorway = sum(st_length(st_intersection(LSOA100[i,], Motorway)))
  LSOA100_A_Road = sum(st_length(st_intersection(LSOA100[i,], A_Road)))
  LSOA100_B_Road = sum(st_length(st_intersection(LSOA100[i,], B_Road)))
  LSOA100_Classified_Road = sum(st_length(st_intersection(LSOA100[i,], Classified_Road)))
  LSOA100_Unclassified_Road = sum(st_length(st_intersection(LSOA100[i,], Unclassified_Road)))
  LSOA100_Not_Classified = sum(st_length(st_intersection(LSOA100[i,], Not_Classified)))
  Addon = data.frame(LSOA21CD = LSOA, Motorway = as.numeric(LSOA100_Motorway), 
                     A_Road = as.numeric(LSOA100_A_Road), 
                     B_Road = as.numeric(LSOA100_B_Road), 
                     Classified_Road = as.numeric(LSOA100_Classified_Road),
                     Unclassified_Road = as.numeric(LSOA100_Unclassified_Road) , 
                     Not_Classified = as.numeric(LSOA100_Not_Classified))
  LSOARoadLengths = rbindlist(list(LSOARoadLengths,Addon))
}

Hectares = data.frame(LSOA21CD = LSOA100$LSOA21CD, Hectares = as.numeric(st_area(LSOA100)/10000))
head(Hectares)
LSOARoadLengths = LSOARoadLengths %>% left_join(Hectares, by = "LSOA21CD")
head(LSOARoadLengths)
LSOARoadLengths = LSOARoadLengths %>% mutate(MotorwayPerHectare = Motorway/Hectares,
...
write.csv(LSOARoadLengths, "./Definitive/LSOA100RoadLengths.csv", row.names = FALSE)


################################################################################

# Now to see whether these coastline LSOA extracts intersect with a BUA

BUA = st_read("./BUA_2022_GB_8703850870315979820.gpkg")


BUA.Areas = data.frame(LSOA21CD = character(), BUA.Area = numeric(), 
                       LSOA300.Area = numeric(), PercentBUA = numeric())

for (i in 1:nrow(LSOA300)){
  print(paste0("Dealing with i = ",i," of ",nrow(LSOA300)," at ",Sys.time()))
  LSOA = LSOA100$LSOA21CD[i]
  BUA.Area = sum(as.numeric(st_area(st_intersection(LSOA300[i,], BUA))))/10000 
  LSOA.Area = as.numeric(st_area(LSOA300[i,]))/10000 
  Proportion = BUA.Area/LSOA.Area
  Addon = data.frame(LSOA21CD = LSOA, BUA.Area = BUA.Area, 
                     LSOA300.Area = LSOA.Area, PercentBUA = round(Proportion, digits=6))
  BUA.Areas = rbindlist(list(BUA.Areas,Addon))
}  
  
write.csv(BUA.Areas, "./Definitive/LSOA300BUA_Areas.csv", row.names = FALSE)



BUA.Areas = data.frame(LSOA21CD = character(), BUA.Area = numeric(), 
                       LSOA200.Area = numeric(), PercentBUA = numeric())

for (i in 1:nrow(LSOA200)){
  print(paste0("Dealing with i = ",i," of ",nrow(LSOA200)," at ",Sys.time()))
  LSOA = LSOA100$LSOA21CD[i]
  BUA.Area = sum(as.numeric(st_area(st_intersection(LSOA200[i,], BUA))))/10000 
  LSOA.Area = as.numeric(st_area(LSOA200[i,]))/10000 
  Proportion = BUA.Area/LSOA.Area
  Addon = data.frame(LSOA21CD = LSOA, BUA.Area = BUA.Area, 
                     LSOA200.Area = LSOA.Area, PercentBUA = round(Proportion, digits=6))
  BUA.Areas = rbindlist(list(BUA.Areas,Addon))
}  

write.csv(BUA.Areas, "./Definitive/LSOA200BUA_Areas.csv", row.names = FALSE)



BUA.Areas = data.frame(LSOA21CD = character(), BUA.Area = numeric(), 
                       LSOA100.Area = numeric(), PercentBUA = numeric())

for (i in 1:nrow(LSOA100)){
  print(paste0("Dealing with i = ",i," of ",nrow(LSOA100)," at ",Sys.time()))
  LSOA = LSOA100$LSOA21CD[i]
  BUA.Area = sum(as.numeric(st_area(st_intersection(LSOA100[i,], BUA))))/10000 
  LSOA.Area = as.numeric(st_area(LSOA100[i,]))/10000 
  Proportion = BUA.Area/LSOA.Area
  Addon = data.frame(LSOA21CD = LSOA, BUA.Area = BUA.Area, 
                     LSOA100.Area = LSOA.Area, PercentBUA = round(Proportion, digits=6))
  BUA.Areas = rbindlist(list(BUA.Areas,Addon))
}  

write.csv(BUA.Areas, "./Definitive/LSOA100BUA_Areas.csv", row.names = FALSE)





  as.numeric(st_area(LSOA300[i,]))/10000                       # 6.2930
  LSOARoadLengths$Hectares[i]                                  # 6.2930

LSOA300List = LSOA300 %>% st_drop_geometry() %>% select(LSOA21CD)
Test = data.frame(LSOA21CD = LSOA300List, Motorway = LSOA300_Motorway, A_Road = LSOA300_A_Road,
                  B_Road = LSOA300_B_Road, Classified_Road = LSOA300_Classified_Road,
                  Unclassified_Road)
Test %>% filter(!
                  Motorway)

as.logical(lengths(Found))
as.logical(lengths(NotFound))

st_intersects(LSOA300[1,], Classified_Road)
st_intersects(LSOA300[1,], Unclassified_Road)

mapview(LSOA300[2,])


RoadNodes = st_read("./oproad_gpkg_gb/Data/oproad_gb.gpkg", layer = "road_node")
head(RoadNodes) # just has junction and road end as points
A_Road = RoadLinks %>% dplyr::filter(road_classification  == "A Road")
...
# So new we need to categorise the PoI 
head(PoI_CoastLine100_LSOAs)
PoI_CoastLine100_LSOAs %>% st_drop_geometry() %>% 



# First - extract coastline LSOAs and PoI in Region[1] 
i=1
WorkingLSOA100 = st_intersection(LSOA100,Region_BFC[i,])
WorkingLSOA200 = st_intersection(LSOA200,Region_BFC[i,])
WorkingLSOA300 = st_intersection(LSOA300,Region_BFC[i,])
mapview(WorkingLSOA300) + mapview(WorkingLSOA200) + mapview(WorkingLSOA100)

WorkingPoI = st_intersection(ONS_PoI,Region_BFC[2,])
WorkingLSOAPoI300 = dplyr::left_join(WorkingPoI, WorkingLSOA300)

Retail_PoI_LSOA21_BUA22 = st_join(Retail_PoI_LSOA21, BUA22, left = TRUE) # this is FALSE so we get NA if not in BUA


  st_intersection(WorkingPoI,Region_BFC[2,])
  
Subset = ONS_PoI[1:10,]
Test <- st_intersects(Region_BFC, Subset)
TestOut = st_intersection(Subset,Region_BFC[2,])
names(TestOut)
...
# (b) Create a narrow (100 metre) buffer on the inside of the coastline
buffer_100 = st_buffer(Dissolved_Coastline_BFC, -100)
buffer_Out100 = st_buffer(Dissolved_Coastline_BFC, 100)
buffer = st_difference(buffer_Out100,buffer_100) 
st_write(buffer, "./Definitive/BFC_CoastlineBuffer_100.gpkg", append=FALSE)
#buffer = st_read("./Definitive/BFC_CoastlineBuffer_50.gpkg")
#mapview(buffer)
...
#st_write(buffer, "./Definitive/BGC_CoastlineBuffer_50.gpkg", append=FALSE)
buffer = st_read("./Definitive/BGC_CoastlineBuffer_50.gpkg")
#mapview(buffer)
...
# (b) Create a narrow (100 metre) buffer on the inside of the coastline
buffer_100 = st_buffer(Dissolved_Coastline_BSC, -100)
bufferOut100 = st_buffer(Dissolved_Coastline_BSC, 100)
buffer = st_difference(bufferOut100,buffer_100) 
#st_write(buffer, "./Definitive/BSC_CoastlineBuffer_50.gpkg", append=FALSE)
buffer = st_read("./Definitive/BSC_CoastlineBuffer_50.gpkg")
mapview(buffer)
...
write.csv(LSOA21_BSC_Dist0_List,"./Definitive/LSOA21_BSC_Dist0_List.csv")
